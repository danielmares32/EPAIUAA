...
class DashboardWindow(QWidget):
    def __init__(self):
        ...
        btn_nav = QPushButton("🔍 Perfiles de Navegación")
        ...
        btn_link = QPushButton("Crear nuevo PLE") #Modificado 20-10-2025
        btn_link.setStyleSheet(
            "background-color: #5a155b; color: white;" #Modificado 20-10-2025 hashtag444
    ...
    def load_ples(self):
        ...
        try:
            r = requests.get(f"{API_BASE}/PLE/user/{user_id}", headers=get_auth_headers(), timeout=10)
            r.raise_for_status()
            data = r.json()
        except Exception as e:
    ...
    def go_to_login(self):
        ...
        if GlobalState.tracking_armed and GlobalState.tracking_mode == "logout":
            ...
            self._hist_worker.done.connect(self._finish_logout)
            self._hist_worker.fail.connect(lambda err: (print("Hist err:", err), self._finish_logout()))
