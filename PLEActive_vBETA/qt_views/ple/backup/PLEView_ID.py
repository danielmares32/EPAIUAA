...
#DEFAULT_PLE_USER_ID = 295  # Usuario por defecto si no se recibe uno Modificado por Equipo UAA 18-08-2025
...
class PLEView(QWidget):
    ...
    def confirmar_seguimiento(self):
        """
        Extrae keywords del historial de Chrome y guarda JSON.
        """
        ...
        resultado = []
        for entry in urls:
            kws = self.extract_all_keywords(entry.get('title','') or entry.get('url',''), top_n=5)
    ...
    def on_second_checkbox_clicked(self, checked):
        ...
        if checked:
            try:
                ...
                self.sites_keywords_window = SitesKeywordsSyncWidget(self)
