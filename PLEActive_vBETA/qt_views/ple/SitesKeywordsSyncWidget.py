...
# Safely wrap stdout/stderr only if not already wrapped
...
# Get base directory - handle both development and PyInstaller bundle
if getattr(sys, 'frozen', False):
    # Running in PyInstaller bundle
    base_dir = Path(sys._MEIPASS)
else:
    # Running in development - go up from qt_views/ple to project root
    base_dir = Path(__file__).parent.parent.parent

print(f"Base directory: {base_dir}")

# Build cross-platform path to perfil_usuario.json
perfil_usuario_path = base_dir / "app" / "auth" / "perfil_usuario.json"
print(f"Looking for perfil_usuario.json at: {perfil_usuario_path}")

clave_a_extraer = 'uid'
valor = obtener_valor_json(str(perfil_usuario_path), clave_a_extraer)
...
if valor:
    ...
    try:
        ...
        print(f"Error: No se pudo convertir user ID '{userIDThisFile}' a entero: {e}")
        GOT_IT_ID = 1  # Default fallback
        print(f"Usando ID por defecto: {GOT_IT_ID}")
else:
...
# Load PLE ID from guardarIDPLE.txt
guardar_id_ple_path = DEFAULT_EXPORT_DIR / "guardarIDPLE.txt"
variable_IDPLE = None
idpleDinamico = 1  # Default fallback

if guardar_id_ple_path.exists():
    try:
                ...
                if "=" in linea:
                    ...
                    if clave == "pleseleccionado":
                        ...
                        try:
                            ...
                            print(f"Error: No se pudo convertir PLE ID '{variable_IDPLE}' a entero: {e}")
                            idpleDinamico = 1  # Default fallback
                            print(f"Usando PLE ID por defecto: {idpleDinamico}")
                        break
    except Exception as e:
        print(f"Error al leer guardarIDPLE.txt: {e}")
        idpleDinamico = 1  # Default fallback
else:
    print(f"Archivo guardarIDPLE.txt no encontrado en: {guardar_id_ple_path}")
    print(f"Usando PLE ID por defecto: {idpleDinamico}") 
...
class ChromeHistoryExtractor:
    ...
    def _resolve_history_for_profile(self, profile_dir):
        if not profile_dir:
            return None

        # If a full path to a History file was provided
        if os.path.isfile(profile_dir):
            return profile_dir

        # If a path to profile directory was provided
        if os.path.isdir(profile_dir):
            hist_path = os.path.join(profile_dir, 'History')
            if os.path.isfile(hist_path):
                return hist_path

        # Otherwise treat as folder name (Default, Profile 1, etc.)
        ...
        candidate = os.path.join(base, profile_dir, 'History')
        if os.path.isfile(candidate):
            return candidate

        if system not in ('Windows', 'Darwin'):
            alt_base = os.path.join(os.path.expanduser('~'), '.config', 'chromium')
            candidate = os.path.join(alt_base, profile_dir, 'History')
            if os.path.isfile(candidate):
                return candidate

        # Try alias mappings (Persona N → Profile N)
        if profile_dir.lower().startswith("persona "):
            try:
                n = int(profile_dir.split()[-1])
                candidate = os.path.join(base, f"Profile {n}", 'History')
                if os.path.isfile(candidate):
                    return candidate
            except Exception:
                pass
    ...
    def get_chrome_history_path(self):
        ...
        # First, try to resolve using selected profile (if provided)
        resolved = self._resolve_history_for_profile(self.profile_dir)
...
class SyncWorkerThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            headers = get_auth_headers()  # Gets Bearer token from config
            headers.update({
                'accept': 'application/json',
                'Content-Type': 'application/json'
            })

            print("🌐 Sending POST request with Bearer authentication...")
...
class SitesKeywordsSyncWidget(QMainWindow):
    ...
    def __init__(
        ...
        # Resolve selected Chrome profile path
        self.selected_profile = selected_profile or getattr(GlobalState, "selected_profile", None)
        self.profile_hint = profile_hint
        if not self.profile_hint:
            if isinstance(self.selected_profile, dict):
                self.profile_hint = (
                    self.selected_profile.get("path")
                    or self.selected_profile.get("profile_path")
                    or self.selected_profile.get("directory")
                    or self.selected_profile.get("name")
                )
            elif isinstance(self.selected_profile, str):
                self.profile_hint = self.selected_profile
        if not self.profile_hint:
            self.profile_hint = (
        ...
        self.chrome_history_path = chrome_history_path
        self.chrome_extractor = ChromeHistoryExtractor(self.profile_hint)
    ...
    def extract_from_chrome(self):
        ...
        try:
            # Refresh profile hint from GlobalState before extraction
            current_profile = getattr(GlobalState, "selected_profile_path", None)
            if not current_profile:
                sel = getattr(GlobalState, "selected_profile", None)
                if isinstance(sel, dict):
                    current_profile = (
                        sel.get("path")
                        or sel.get("profile_path")
                        or sel.get("directory")
                        or sel.get("name")
                    )
                elif isinstance(sel, str):
                    current_profile = sel
            if not current_profile:
                current_profile = getattr(GlobalState, "current_profile_dir", None)

            self.profile_hint = current_profile or self.profile_hint
            self.chrome_extractor.set_profile_dir(self.profile_hint)
            ...
            profile_label = None
            if isinstance(self.profile_hint, str) and self.profile_hint:
                profile_label = os.path.basename(self.profile_hint.rstrip("\\/"))
                if not profile_label:
                    profile_label = self.profile_hint
            if not profile_label:
                profile_label = "Default"
            self.status_label.setText(f"Conectando a Chrome ({profile_label})...")
